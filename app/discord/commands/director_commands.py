...
class DiscoveryChoiceButton(discord.ui.Button):
    ...
    def __init__(self, discovery_id: str, action_choice: str):
        super().__init__(
            style=discord.ButtonStyle.secondary,
            label=action_choice[:80],
            emoji="👉"
        )
...
class DiscoveryActionView(discord.ui.View):
    ...
    def __init__(self, discovery_id: str, actions: List[str]):
        super().__init__(timeout=180)
        for act in actions[:5]:
            self.add_item(DiscoveryChoiceButton(discovery_id=discovery_id, action_choice=act))
...
async def home_command(interaction: discord.Interaction):
    ...
    try:
        ...
        char = await character_service.get_active_character_by_user(user_id)
        if not char:
            await interaction.followup.send("No active character. Use `/start` first.", ephemeral=True)
            return
        ...
        logger.error(f"Error in home_command: {e}", exc_info=True)
        await interaction.followup.send(f"⚠️ Error loading home chronicle: {e}", ephemeral=True)
...
async def opportunities_command(interaction: discord.Interaction):
    ...
    try:
        ...
        if not opps:
            await interaction.followup.send("The docks are quiet at this exact moment. Try `/explore`.", ephemeral=True)
            return
        ...
        await interaction.followup.send(f"⚠️ Error retrieving opportunities: {e}", ephemeral=True)
...
async def threads_command(interaction: discord.Interaction):
    ...
    try:
        ...
        if not threads:
            await interaction.followup.send("You are not currently entangled in any active story threads.", ephemeral=True)
            return
        ...
        logger.error(f"Error in threads_command: {e}", exc_info=True)
        await interaction.followup.send(f"⚠️ Error retrieving story threads: {e}", ephemeral=True)
...
async def people_command(interaction: discord.Interaction):
    ...
    try:
        ...
        if not people:
            await interaction.followup.send("You haven't made any lasting impressions on the people of this port yet.", ephemeral=True)
            return
        ...
        logger.error(f"Error in people_command: {e}", exc_info=True)
        await interaction.followup.send(f"⚠️ Error retrieving relationships: {e}", ephemeral=True)
...
async def explore_command(interaction: discord.Interaction):
    ...
    try:
        ...
        await interaction.followup.send(f"⚠️ Error exploring surroundings: {e}", ephemeral=True)
...
async def goals_command(interaction: discord.Interaction):
    ...
    try:
        ...
        logger.error(f"Error in goals_command: {e}", exc_info=True)
        await interaction.followup.send(f"⚠️ Error retrieving ambitions: {e}", ephemeral=True)
